...
export const ExpandableSkillTags = ({
  ...
  return (
    <section className={cn("w-full", className)}>
      <h3 className="mb-4 text-xs font-semibold text-cyan-400 uppercase tracking-wider">{title}</h3>
      <motion.div
      ...
      >
        ...
        {visibleSkills.map((skill, index) => (
          <motion.div key={`visible-${index}`} variants={itemVariants}>
            <Badge variant="secondary" className="px-3 py-1 bg-zinc-900/80 border border-zinc-800 text-zinc-300 hover:bg-zinc-800 font-normal">{skill}</Badge>
          </motion.div>
        ))}
      ...
      {skills.length > initialCount && (
        <Button
          variant="link"
          className="mt-3 px-0 text-sm text-cyan-500 hover:text-cyan-400"
          onClick={toggleExpansion}
          aria-expanded={isExpanded}
        >
          {isExpanded ? "View less" : `+${skills.length - initialCount} more`}
        </Button>
      )}
    </section>
  );
